...
for line in data:
    ...
    nom,R,G,B = cont2
    Lnom += [nom]
    dico[nom] = [int(R),int(G),int(B)]
    
print('Red =',dico['Red'])
print('Lime =',dico['Lime'])
print('Blue =',dico['Blue'])
    
    
def patchwork (L,dico) :
    
    """arg : liste des couleurs et structure donnant leur code
    renvoi : patchwork """
    
    n = int(int(np.sqrt(len(L))+1)**2)-len(L) # pas opti mais passé trop de temps
    
    compl = ['White']*(n)  # on complète pour avoir une len carrée
    colors = np.array([dico[name] for name in L+compl],dtype = np.uint8)
    P= np.zeros((len(colors),1,3),dtype = np.uint8)
    P[:,0] = colors
    newP = P.reshape((int(np.sqrt(len(colors))),int(np.sqrt(len(colors))),3))
    plt.imshow(newP)
    plt.show()
    return newP
    
image_final = patchwork(Lnom , dico)


Lrand = [Lnom[np.random.randint(0 , len(Lnom))] for k in range(78)]


patchwork(Lrand, dico)
print(np.shape(image_final))
plt.imsave('patchwork.jpg' , image_final)
...
# np.sum gère le cas de dépassement du uint8

gris = copie.mean(axis = 2)
...
# votre code
def sepia (RGB) :
    M = np.array([[0.393,0.769,0.189],[0.349,0.686,0.168],[0.272,0.534,0.131]],dtype = np.float64)
    return np.dot((RGB/255),M.transpose())

sepia1 = sepia(image_final)
...
#noir donc les images sont proches

plt.imsave('newmines2.jpg',im2)
im1.save('newmines1.jpg', quality = 100)

#l'image via save est plus lourde

im3 = plt.imread('newmines2.jpg')
im4 = plt.imread('newmines1.jpg')
