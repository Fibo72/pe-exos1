...
plt.plot(np.arange(0,len(df['Age'])) , df['Age'] , 'b')
...
# les deux passagers ont le même age

df1 = df.sort_values(['Age','Fare'])

manquante = df1[df1['Age'].isna()]
manquante.head(3)
...
# Les passagers sans age sont à la fin

# #df.sort_values?

df2 = df1.sort_values(['Age','Fare'], na_position = 'first' )

print(df2.head(5))


présent = df[df['Age'].notna()].sort_values(['Age','Fare'])
présent.head(5)
...
axl = np.random.randint(0 , 100 , size = (4,5))
df = pd.DataFrame(axl , index = ['un' , 'deux' , 'trois' , 'quatre'] , columns = ['a' , 'b' , 'c' , 'd', 'e'])
print(df)

tri = df.sort_values('trois', axis = 1)
