...
empty_columns = df.columns[is_empty_column(df , df.columns)]

df.drop(labels = empty_columns, axis = 'columns' , inplace = True)
```
...
# pb de comparaison en entier car Nan ne se compare pas
```
...
```{code-cell} ipython3
# à vous
unique2 = {k for k in df['cLT2FREQ']}
```
...
```{code-cell} ipython3
# à vous
nb_lines_to_clean = len(df['cLT2FREQ'][np.isnan(df.cLT2FREQ)])
```
...
```{code-cell} ipython3
# à vous
df.drop(df.index[np.isnan(df.cLT2FREQ)] , inplace = True)
df
```
...
df.to_excel(excel_writer  = df , sheet_name = filename)
```
